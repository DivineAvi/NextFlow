...
function TemplateCard({ template }: { template: (typeof WORKFLOW_TEMPLATES)[0] }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <div
        className="group relative flex aspect-video w-full cursor-pointer rounded-xl bg-zinc-950 border border-zinc-800 hover:border-zinc-600 transition-all duration-200 overflow-hidden"
        onClick={useTemplate}
      >
        <WorkflowMinimap nodes={template.nodes as any} edges={template.edges as any} />
        {/* Hover overlay */}
                ...
                <div key={workflow.id} className="flex flex-col gap-2">
                  <div className="relative group">
                    <Link
                      ...
                      className="flex aspect-video w-full rounded-xl bg-zinc-950 border border-zinc-800 hover:border-zinc-600 transition-all duration-200 overflow-hidden relative"
                    >
                      <WorkflowMinimap
